...
impl Proc {
    ...
    // Helper method to check if proc has a field in fields list - matches JavaCC hasFields()
    ...
    pub fn has_update_fields(&self, field_name: &str) -> bool {
        // In JavaCC, this checks proc.updateFields
        // For now, we'll check if it's in the main fields list and is an update procedure
        self.is_update && self.fields.contains(&field_name.to_string())
    }
    
    // Helper method to check if proc has order fields - matches JavaCC hasOrders()
    pub fn has_orders(&self, field_name: &str) -> bool {
        // In JavaCC, this checks proc.orderFields
        // For now, we'll check if it's in the main fields list
        self.fields.contains(&field_name.to_string())
    }
    
    // Helper method to check if dynamic already exists - matches JavaCC hasDynamic()
    pub fn has_dynamic(&self, dynamic_name: &str) -> bool {
        self.dynamics.contains(&dynamic_name.to_string())
    }
}
...
fn parse_proc_section(pair: pest::iterators::Pair<Rule>) -> Result<Proc, Box<dyn std::error::Error>> {
    ...
    for inner_pair in pair.into_inner() {
        match inner_pair.as_rule() {
            ...
            Rule::proc_body => {
                // Handle procedure body - this could be a user procedure
                let mut has_user_proc_content = false;
                ...
                // If we found user procedure content, mark as user procedure
                if has_user_proc_content && proc_type.is_empty() {
                    proc_type = "USER".to_string();
                    proc.is_built_in = false;
                }
            }
            _ => {}
    ...
    // Handle user-defined procedures (jUserProc equivalent) when no built-in pattern matched
    if proc_type.is_empty() && !proc.is_built_in {
        proc_type = "USER".to_string();
        // The procedure name is already set from the identifier parsing above
        // This matches JavaCC jUserProc() behavior where proc.name = s (from jIdent())
    }
...
fn parse_jrow_count(pair: pest::iterators::Pair<Rule>) -> Result<i32, Box<dyn std::error::Error>> {
    for inner_pair in pair.into_inner() {
        if let Rule::number = inner_pair.as_rule() {
            return Ok(inner_pair.as_str().parse().unwrap_or(0));
        }
    }
    Ok(0)
}
