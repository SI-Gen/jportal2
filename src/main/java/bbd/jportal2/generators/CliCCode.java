...
public class CliCCode extends BaseGenerator implements IBuiltInSIProcessor {
    ...
    private void generateSnips(Table table, String output) throws Exception {
        try (PrintWriter outData = this.openOutputFileForGeneration("_snips.h", output + table.useName().toLowerCase() + "_snips.h")) {
            outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
    ...
    private void generateSnipsAction(Table table, Proc proc, PrintWriter outData) {
        String dataStruct;
    ...
    private void generateSnipsBulkAction(Table table, Proc proc, PrintWriter outData) {
        String dataStruct;
        if (proc.isStd || proc.isStdExtended())
            dataStruct = table.useName();
        else
    ...
    private void generateSnipsSingle(Table table, Proc proc, PrintWriter outData) {
        String dataStruct;
        if (proc.isStd || proc.isStdExtended())
            dataStruct = table.useName();
        else
    ...
    private void generateSnipsMultiple(Table table, Proc proc, PrintWriter outData) {
        String dataStruct;
        if (proc.isStd || proc.isStdExtended())
            dataStruct = table.useName();
        else
     ...
     */
    //Vector<Field> nullVectorF = new Vector<Field>();
    Vector<String> nullVectorS = new Vector<String>();
    String structName = "";
    ...
    private void headerSwaps(PrintWriter outData, String baseClass, Vector<Field> inputs, Proc proc) {
    ...
    private void extendHeader(PrintWriter outData, String baseClass, Vector<Field> inputs, String useName, Vector<String> dynamics, Proc proc) {
     ...
     * Emits class method for processing the database activity
     */
    boolean isMsSqlIdentity(Field field) {
    ...
    void generateWithParms(Proc proc, PrintWriter outData, String pad) {
        ...
        for (int j = 0; j < proc.inputs.size(); j++) {
            Field field = (Field) proc.inputs.elementAt(j);
            if ((isSequence(field) && proc.isInsert) || isMsSqlIdentity(field)
