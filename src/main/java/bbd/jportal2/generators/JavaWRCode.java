...
public class JavaWRCode extends BaseGenerator implements IBuiltInSIProcessor {

    private static final Logger logger = LoggerFactory.getLogger(JavaWRCode.class);
    public static final String GENERATE_PROCS_IO_ERROR = "Generate Procs IO Error";

    public JavaWRCode() {
        super(JavaWRCode.class);
    }
    ...
    private void generate(Table table, String output) {
        ...
        try (PrintWriter outFile = openOutputFileForGeneration("Java",
                output + table.useName() + ".java")) {
            logger.info("Code: {}{}.java", output, table.useName());

            PrintWriter outData = new PrintWriter(outFile);
            generateStdProcs(table, outData);
            generateOtherProcs(table, outData);
            outData.println("}");
            outData.flush();
     ...
     */
    private void generateStdCopiers(Table table, PrintWriter outData) {
        ...
        for (int i = 0; i < table.fields.size(); i++) {
            ...
            if (field.isNull && field.isEmptyOrAnsiAsNull() == false)
                outData.println("    " + name + "IsNull = _value." + name + "IsNull;");
        }
        outData.println("  }");
    }

    private void generateStdProcs(Table table, PrintWriter outData) {
        ...
        if (table.fields.size() > 0) {
            ...
            outData.println("  {");
            outData.println("    private final long serialVersionUID = 1L;");
            outData.println("    /**");
    ...
    private void generateOtherCopiers(Table table, Proc proc, PrintWriter outData) {
        String extendsName = table.useName() + "Rec";
     ...
     */
    private void generateOtherProcs(Table table, PrintWriter outData) {
        for (int i = 0; i < table.procs.size(); i++) {
            Proc proc = (Proc) table.procs.elementAt(i);
            if (proc.isData)
                continue;
     ...
     *
     */
    private int checkPlaceHolders(Proc proc, PrintWriter outData, String l, String parmField, int phIndex) {
    ...
    private void checkForExclusion(PrintWriter outData, String l, String parmField) {
    ...
    private void emitOutputBinds(Proc proc, Field field, String prep, String indent, int pos, PrintWriter outData) {
        if (field.isOut == true) {
    ...
    private void emitInputBinds(Proc proc, Field field, String prep, String indent, int pos, PrintWriter outData) {
    ...
    private String possibleAnd(Field field, int no) {
        if (field.type == Field.ANSICHAR && field.length == 1)
     ...
     */
    private String getParmField(Proc proc) {
    ...
    private void generateStdProcStruct(Table table, String output) {
        ...
        String outFileName = output + table.useName() + "Rec.java";
        logger.info("Code: {}", outFileName);
        try (PrintWriter outData = openOutputFileForGeneration("Java", outFileName)) {
            outData.println("// This code was generated, do not modify it, modify it at source and regenerate it.");
            if (table.database.packageName.length() > 0) {
                outData.println("package " + table.database.packageName.toLowerCase() + ";");
                outData.println();
    ...
    private void generateOtherProcStruct(Table table, Proc proc, String output) {
        ...
        String outFileName = output + table.useName() + proc.upperFirst() + "Rec.java";
        logger.info("Code: {}", outFileName);
        try (PrintWriter outData = openOutputFileForGeneration("Java", outFileName)) {
            if (table.database.packageName.length() > 0) {
                outData.println("package " + table.database.packageName.toLowerCase() + ";");
    ...
    private String javaType(Field field) {
        switch (field.type) {
            case Field.BYTE:
